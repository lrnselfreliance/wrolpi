...
class TestDrivesConfig:
    ...
    @pytest.mark.parametrize("filesystem", ["ext4", "btrfs", "vfat", "exfat", "zfs"])
    ...
    @pytest.mark.parametrize("key", ["mounts", "zfs_pools"])
    def test_empty_lists_by_default(self, key):
        """Lists should be empty by default."""
        assert DEFAULT_CONFIG["drives"][key] == []
...
class TestManagedServicesConfig:
    ...
    @pytest.mark.parametrize("name,expected", [
        ("wrolpi-api", {"systemd_name": "wrolpi-api", "port": 8081, "viewable": True, "description": "Python API (Sanic)"}),
