...
function MapPlaceCard({place, index}) {
    ...
    const media = <div style={{height: 150, overflow: "hidden", position: "relative", background: "var(--head)"}}>
        {iframeSrc && <iframe
            src={iframeSrc}
            title={place.name}
            loading="lazy"
    ...
    return <Card
        ...
        media={media}
        title={place.name}
        meta={subtitle.length > 0 ? subtitle.join(" · ") : undefined}
    >
        <div>{place.lat.toFixed(4)}, {place.lon.toFixed(4)}</div>
    </Card>;
}
...
export function MapSearchView() {
    ...
    if (results.length === 0) {
        return <Panel><Header as="h4">No map locations found for "{searchStr}".</Header></Panel>;
    }
