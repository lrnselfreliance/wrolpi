...
async def test_files_search_by_suffix(test_session, async_client, make_files_structure):
    ...
    async def search_suffix(suffix):
        request, response = await async_client.post('/api/files/search', json=dict(suffix=suffix))
        assert response.status_code == HTTPStatus.OK
    ...
    assert await search_suffix('.bin') == ['software/MALVEKE.ino.bin']
    assert await search_suffix('bin') == ['software/MALVEKE.ino.bin']
    assert await search_suffix('.BIN') == ['software/MALVEKE.ino.bin']
    assert await search_suffix('.uf2') == ['software/firmware-heltec.uf2', 'software/firmware-r1-neo.uf2']
    assert await search_suffix('.txt') == ['notes.txt']
    # A suffix that matches nothing returns no results.
    assert await search_suffix('.gguf') == []
...
@pytest.mark.asyncio
async def test_move(test_session, test_directory, make_files_structure, async_client, await_background_tasks):
...
@pytest.mark.asyncio
async def test_rename_file(test_session, test_directory, make_files_structure, async_client, refresh_files):
...
@pytest.mark.asyncio
async def test_rename_directory(test_session, test_directory, make_files_structure, async_client, refresh_files):
    make_files_structure({
        'foo/bar/baz.txt': 'asdf',
    })
    await refresh_files()
...
@pytest.mark.asyncio
async def test_rename_directory_with_nested_collections(test_session, test_directory, async_client, refresh_files):
...
@pytest.mark.asyncio
async def test_get_file(test_session, async_client, test_directory, make_files_structure, refresh_files):
