...
class TestCaption(TestAPI):
    ...
    def test_process_captions(self):
        with get_db_session(commit=True) as session:
            ...
            session.add(video1)
            with mock.patch('modules.videos.captions.get_absolute_video_caption', lambda *a: self.vtt_path1):
                captions.process_captions(video1)
            video2 = Video(title='bar', caption_path=str(self.vtt_path2))
            session.add(video2)
            with mock.patch('modules.videos.captions.get_absolute_video_caption', lambda *a: self.vtt_path2):
