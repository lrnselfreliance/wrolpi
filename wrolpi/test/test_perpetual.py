...
def _wait_for_byte(fd: int, timeout: float) -> bool:
    ...
    return bool(ready) and bool(os.read(fd, 1))
...
def test_run_perpetual_process_exits_cleanly_on_signal(async_client, monkeypatch, sig):
    ...
    process.start()
    try:
        assert _wait_for_byte(ready_r, 10), 'the perpetual process did not start its loop'
...
@pytest.mark.asyncio
async def test_reconcile_after_predecessor(async_client, test_session, monkeypatch):
