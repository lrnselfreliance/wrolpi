...
async def test_get_video(async_client, test_session, simple_channel, video_factory):
    ...
    now_ = now()
    video1 = await video_factory(channel_id=simple_channel.id, title='vid1')
    video1.file_group.published_datetime = now_
    video2 = await video_factory(channel_id=simple_channel.id, title='vid2')
...
@pytest.mark.asyncio
async def test_get_video_slug(async_client, test_session, video_factory):
    """Can fetch a Video by its slug."""
    video = await video_factory()
    request, response = await async_client.get(f'/api/videos/video/{video.slug}')
    assert response.status_code == HTTPStatus.OK
...
async def test_channel_download_requires_refresh(
    ...
    vid = await video_factory(channel_id=download_channel.id, with_video_file=True, with_poster_ext='jpg')
...
@pytest.mark.asyncio
async def test_search_videos_channel(async_client, test_session, video_factory):
    ...
    with get_db_session(commit=True) as session:
        vid1 = await video_factory(title='vid1', channel_id=channel2.id)
        vid2 = await video_factory(title='vid2', channel_id=channel1.id)
        session.add(vid1)
        session.add(vid2)
...
async def test_get_channel_videos_pagination(test_session, simple_channel, video_factory, assert_video_search):
    for i in range(50):
        await video_factory(channel_id=simple_channel.id)
    ...
    await video_factory(title='vid2', channel_id=channel2.id, with_video_file=True)
    test_session.commit()
...
async def test_tag_channel(async_client, test_session, test_directory, channel_factory, tag_factory, video_factory,
                           test_channels_config, test_download_manager, test_downloader, await_tasks):
    ...
    v1 = await video_factory(title='video1', channel_id=channel.id)
    v2 = await video_factory(title='video2', channel_id=channel.id)
    # Create recurring download which uses the Channel's directory.
    download = download_manager.recurring_download(
        'https://example.com/1', 60, test_downloader.name,
        settings={'destination': str(test_directory / 'videos/Channel Name')})
    test_session.commit()
...
async def test_move_channel(async_client, test_session, test_directory, channel_factory, tag_factory,
    ...
    vid1 = await video_factory(channel_id=channel.id)
    vid2 = await video_factory(channel_id=channel.id)
    test_session.commit()
