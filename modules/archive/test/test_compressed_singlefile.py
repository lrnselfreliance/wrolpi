...
from modules.archive.lib import (
    ...
    inline_compressed_singlefile_resources,
    is_singlefile_file,
    write_archive_files,
)
...
def test_compressed_singlefile_helpers(compressed_singlefile_factory, singlefile_contents_factory):
    """Compressed singlefiles are detected, and their page/URL can be extracted."""
...
def find_singlefile_path(written):
    return next(i for i in written.paths if i.suffix == '.html' and '.readability' not in i.name)
...
def test_write_archive_files_compressed(test_directory, test_session, image_bytes_factory,
    ...
    readability files are written uncompressed, with their ZIP image references inlined."""
    compressed = compressed_singlefile_factory()
    readability = dict(content='<html><img src="images/0.png">readability content</html>',
                       textContent='readability text', title='a title')
    ...
    singlefile_path = find_singlefile_path(written)
    ...
    # Readability files are plain text, not compressed; their ZIP image references are inlined.
    readability_path = next(i for i in written.paths if i.name.endswith('.readability.html'))
    readability_text = readability_path.read_text()
    assert 'readability content' in readability_text
    assert 'data:image/png;base64,' in readability_text
    assert 'src="images/0.png"' not in readability_text
    ...
    # A singlefile marked compressed (<html data-sfz) whose ZIP is unreadable (e.g. truncated) is
    ...
    assert find_singlefile_path(written).read_bytes() == broken


def test_inline_compressed_singlefile_resources(compressed_singlefile_factory):
