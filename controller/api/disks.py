...
@router.get("/detect-wrolpi")
async def detect_wrolpi_drives():
    """Detect unmounted WROLPi drives."""
    _check_native_mode()

    drives = scan_for_wrolpi_drives()
    return [
        {
            "name": d.name,
            "path": d.path,
            ...
            "label": d.label,
            "uuid": d.uuid,
            "is_wrolpi_drive": d.is_wrolpi_drive,
        }
        for d in drives
    ]
...
@router.get("/smart/{device}")
async def get_smart(device: str):
    """Get SMART status for a specific drive."""
    _check_native_mode()

    if not is_smart_available():
        raise HTTPException(status_code=501, detail="SMART not available")

    # Convert device name to path
    if not device.startswith("/dev/"):
        device = f"/dev/{device}"

    status = get_smart_status(device)
    if status is None:
...
@router.post("/zfs/pools/{name}/import")
async def zfs_import(name: str):
    """Import a ZFS pool."""
    _check_native_mode()

    result = import_zfs_pool(name)
    if not result.get("success"):
...
@router.post("/zfs/pools/{name}/export")
async def zfs_export(name: str):
    """Export a ZFS pool."""
    _check_native_mode()

    result = export_zfs_pool(name)
    if not result.get("success"):
