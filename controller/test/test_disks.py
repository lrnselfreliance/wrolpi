...
class TestGetDeviceByUuid:
    """Tests for get_device_by_uuid function."""

    def test_returns_device_path(self):
        """Should return device path for UUID."""
        with mock.patch("subprocess.run") as mock_run:
            mock_run.return_value = mock.Mock(returncode=0, stdout="/dev/sda1\n")
            result = get_device_by_uuid("1234-5678")
            assert result == "/dev/sda1"

    def test_returns_none_if_not_found(self):
        """Should return None if UUID not found."""
        with mock.patch("subprocess.run") as mock_run:
            mock_run.return_value = mock.Mock(returncode=1, stdout="")
            result = get_device_by_uuid("nonexistent")
            assert result is None
...
class TestZfsFunctions:
    """Tests for ZFS-related functions."""

    def test_get_zfs_pools_when_not_available(self):
        """Should return empty list when zpool not available."""
        with mock.patch("shutil.which", return_value=None):
            result = get_zfs_pools()
            assert result == []

    def test_get_zfs_pools_parses_output(self):
        """Should parse zpool list output."""
        zpool_output = "tank\t10T\t5T\t5T\tONLINE\nbackup\t2T\t1T\t1T\tONLINE"
        with mock.patch("shutil.which", return_value="/sbin/zpool"):
            with mock.patch("subprocess.run") as mock_run:
                mock_run.return_value = mock.Mock(returncode=0, stdout=zpool_output)
                result = get_zfs_pools()
                assert len(result) == 2
                assert result[0]["name"] == "tank"
                assert result[0]["health"] == "ONLINE"

    def test_import_zfs_pool_success(self):
        """Should import ZFS pool successfully."""
        with mock.patch("subprocess.run") as mock_run:
            mock_run.return_value = mock.Mock(returncode=0, stderr="")
            result = import_zfs_pool("tank")
            assert result["success"] is True

    def test_import_zfs_pool_failure(self):
        """Should handle ZFS pool import failure."""
        with mock.patch("subprocess.run") as mock_run:
            mock_run.return_value = mock.Mock(returncode=1, stderr="pool not found")
            result = import_zfs_pool("nonexistent")
            assert result["success"] is False
            assert "pool not found" in result["error"]

    def test_export_zfs_pool_success(self):
        """Should export ZFS pool successfully."""
        with mock.patch("subprocess.run") as mock_run:
            mock_run.return_value = mock.Mock(returncode=0, stderr="")
            result = export_zfs_pool("tank")
            assert result["success"] is True
