...
def build_all_in_directory(directory, max_zoom=DEFAULT_MAX_ZOOM):
    ...
    # Skip terrain files — they contain elevation data, not place names.
    pmtiles_files = [p for p in pmtiles_files if 'terrain' not in p.stem.lower()]
...
def main():
    ...
    if path.is_dir():
        if args.output:
            ...
            sys.exit(1)
        build_all_in_directory(path, max_zoom=args.max_zoom)
